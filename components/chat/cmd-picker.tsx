...
export const CmdPicker: FC<CmdPickerProps> = ({}) => {
  const { focusCmd, cmdCommand, isCmdPickerOpen, setIsCmdPickerOpen } =
  ...
  const cmds: Command[] = [
    ...
    },
    {
      ...
      description:
        "Copy the entrie chat history of the current chat in format of plain text"
    }
  ]
            ...
            <>
              {filteredCmds.map((item, index) => (
                <div
                  ...
                  onClick={() => callSelectCmd(item as Tables<"assistants">)}
